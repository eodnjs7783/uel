...
void Test_UEL_APP_TblValidationFunc(void)
{
    /*
     * Test Case For:
    ...
    UtAssert_INT32_EQ(UEL_APP_TblValidationFunc(&TestTblData), CFE_SUCCESS);

    /* error case should return UEL_APP_TABLE_OUT_OF_RANGE_ERR_CODE */
    TestTblData.Int1 = 1 + UEL_APP_TBL_ELEMENT_1_MAX;
    UtAssert_INT32_EQ(UEL_APP_TblValidationFunc(&TestTblData), UEL_APP_TABLE_OUT_OF_RANGE_ERR_CODE);
}

void Test_UEL_APP_GetCrc(void)
{
    /*
     * Test Case For:
...
{
    ADD_TEST(UEL_APP_TblValidationFunc);
    ADD_TEST(UEL_APP_GetCrc);
}
