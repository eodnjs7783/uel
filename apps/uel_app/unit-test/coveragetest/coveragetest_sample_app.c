...
void Test_UEL_APP_Main(void)
{
    ...
    /*
     * UEL_APP_Main does not return a value,
     ...
     * The first is for UEL_APP_Init().  As this is in the same
     ...
     * the UEL_APP_Init() failure path was taken.
     */
    UEL_APP_Main();
     ...
     */
    UtAssert_UINT32_EQ(UEL_APP_Data.RunStatus, CFE_ES_RunStatus_APP_ERROR);
    ...
    UT_CHECKEVENT_SETUP(&EventTest, UEL_APP_PIPE_ERR_EID, "UEL APP: SB Pipe Read Error, App Will Exit");
...
void Test_UEL_APP_Init(void)
{
    ...
    UtAssert_INT32_EQ(UEL_APP_Init(), CFE_SUCCESS);
     ...
     */
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UtAssert_INT32_EQ(UEL_APP_Init(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_STUB_COUNT(CFE_ES_WriteToSysLog, 1);

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UtAssert_INT32_EQ(UEL_APP_Init(), CFE_SB_BAD_ARGUMENT);
    ...
    UtAssert_INT32_EQ(UEL_APP_Init(), CFE_TBL_ERR_INVALID_OPTIONS);
...
{
    ADD_TEST(UEL_APP_Main);
    ADD_TEST(UEL_APP_Init);
}
